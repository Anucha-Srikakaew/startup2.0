...
<head>
    ...
    $strSQL = "SELECT DISTINCT PROCESS FROM `$tbl_item` 
            WHERE `PERIOD` = '$PERIOD' AND LINE = '$LINE' AND TYPE LIKE '$TYPE' AND MODEL LIKE '$MODEL' AND SHIFT_DATE 
            LIKE '$DATE_SHIFT' AND SHIFT LIKE '$SHIFT' ORDER BY ID ASC ";
    $objQuery = mysqli_query($con, $strSQL);

    $COUNT_NUM = 0;
    while ($objResult = mysqli_fetch_array($objQuery)) {
        $PROCESS_SQL[] = $objResult['PROCESS'];
        $process_salfff = $objResult['PROCESS'];
    ...
    // AND LastUpdate = (
    //     SELECT LastUpdate FROM `$tbl_item` WHERE LINE = '$LINE' 
    ...
    <?php if (isset($sub)) { ?>
        ...
        <div class="row text-center" id="SHOW_TABLE">
            <div class="col-lg-12 mx-auto">
                ...
                <h5 class="text-danger"><?php echo $SHOW_TYPE_ ?> : <?php echo $LINE; ?>/ Model: <?php echo $MODEL ?>/ Type : <?php echo $TYPE ?>/ Shift Date : <?php echo $DATE_SHIFT ?></h5>
                <br><br>
...
<script>
    ...
    $(document).ready(function() {
        $('#right_one').on('click', function() {
            ...
            $.get("visual_item.php?LINE=" + LINE + "&PROCESS=" + PROCESS + "&MODEL=" + MODEL + "&TYPE=" + TYPE + "&DATE_SHIFT=" + DATE_SHIFT + "&DATE=" + DATE + "&SHIFT=" + SHIFT, ID_CHECK, processResponse);
            $('#testdiv,#nav-df').hide();
        })
    ...
    function backtoprocess() {
        history.back()
        // console.log('http://43.72.52.51/startup2.0/visual_model.php?MODEL=<?php echo $MODEL ?>&CENTER=<?php echo $LINE; ?>&DATE=<?php echo $DATE; ?>&SHIFT=<?php echo $SHIFT; ?>&DATE_SHIFT=<?php echo $DATE_SHIFT; ?>&BIZ=<?php echo $BIZ ?>&CHECK_PROCESS=YES&TYPE=<?php echo $TYPE ?>')
        // console.log('http://43.72.52.51/startup2.0/visual_model.php?MODEL=LS&CENTER=SOLDERING&DATE=2022-01-14&SHIFT=NIGHT&DATE_SHIFT=2022-01-13')
        // window.location.href = 'http://43.72.52.51/startup2.0/visual_model.php?MODEL=<?php echo $MODEL ?>&CENTER=<?php echo $LINE; ?>&DATE=<?php echo $DATE; ?>&SHIFT=<?php echo $SHIFT; ?>&DATE_SHIFT=<?php echo $DATE_SHIFT; ?>&BIZ=<?php echo $BIZ ?>&CHECK_PROCESS=YES&TYPE=<?php echo $TYPE ?>';

    }

    function backtohome() {
        window.location.href = 'http://43.72.52.51/startup2.0/visual.php?BIZ=AC';
    }
</script>