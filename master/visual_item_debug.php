...
<head>
    ...
    $strSQL = "SELECT DISTINCT PROCESS,BIZ
            ,MODEL,TYPE,SHIFT,LastUpdate FROM `startup_item` 
            WHERE LINE = '$LINE' AND TYPE LIKE '%$TYPE%' AND MODEL LIKE '$MODEL' AND SHIFT_DATE 
    ...
    FROM `startup_item` 
    WHERE `LINE` = '$LINE' AND 
    PROCESS = '$PROCESS_SQL[$PROCESSNEXT]' AND
    `TYPE` LIKE '%$TYPE%' AND 
    `SHIFT_DATE` LIKE '$DATE%'  AND 
    `SHIFT` LIKE '$SHIFT' AND
    LastUpdate  BETWEEN (
        SELECT LastUpdate 
        FROM `startup_item` 
        WHERE ID = (
        ...
        WHERE ID = (
            ...
            `LINE` = '$LINE' AND 
            `TYPE` LIKE '%$TYPE%' AND 
            PROCESS = '$PROCESS_SQL[$PROCESSNEXT]'))
    ORDER BY ID ASC";
    ...
    <?php if (isset($sub)) { ?>
        ...
        <div class="row text-center" id="SHOW_TABLE">
            <div class="col-lg-12 mx-auto">
                ...
                <h3>ITEM : <?php echo $PROCESS_SQL[$PROCESSNEXT]; ?></h3>
                <br><br>
        ...
        <!-- <nav id="nav-df" class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top ">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav mr-auto">
                    ...
                    <li class="nav-item active">
                        <a class="nav-link" href="http://43.72.52.52/startup2.0/login.php?BIZ=<?php echo $BIZ ?>">LOGIN</a>
                    </li>
                    <li class="nav-item active">
                        <a class="nav-link" href="http://43.72.52.52/startup2.0/visual.php?BIZ=<?php echo $BIZ; ?>&LINE=<?php echo $LINE; ?>">STARTUP</a>
                    </li>
                </ul>
                <div class="form-inline my-2 my-lg-0">
                    <a class="nav-link" style="color:white;" href="#" onclick="backtoprocess()">BACK</a>
...
<script>
    ...
    function backtoprocess() {
        window.location.href = 'http://43.72.52.51/startup2.0/visual_line.php?LINE=<?php echo $LINE; ?>&DATE=<?php echo $DATE; ?>&SHIFT=DAY&DATE_SHIFT=<?php echo $DATE_SHIFT; ?>&BIZ=LENS&CHECK_PROCESS=YES&MODEL=<?php echo $MODEL; ?>&TYPE=<?php echo $TYPE; ?>&DATE=<?php echo $DATE; ?>&SHIFT=<?php echo $SHIFT; ?>';
