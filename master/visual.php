...
<body>
    ...
    if ($BIZ == 'BODY') {
        header("location: visual.php?BIZ=IM");
    }
                        ...
                        <tbody>
                            ...
                            if ($PERIOD_GET == 'DAY') {
                                ...
                                if (isset($_GET['STARTUP_DATE']) || isset($_GET['STARTUP_SHIFT'])) {
                                    ...
                                    // echo $DATE;
                                    foreach ($LINE as $LINE_NAME) {
                                        $strSQL = "SELECT `SHIFT_DATE` FROM `$tbl_time` 
                                        WHERE `PERIOD` = '$PERIOD_GET' AND `LINE` = '$LINE_NAME' AND `BIZ` = '$BIZ' AND `SHIFT_DATE` BETWEEN '$start_date' AND '$end_date' ORDER BY `ID` DESC";
                                        $objQuery = mysqli_query($con, $strSQL);
                                        $objResult = mysqli_fetch_array($objQuery);
                                        if (!isset($objResult)) {
                                            $CHECK_SHIFT_DATE[] = '0';
