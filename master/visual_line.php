...
if ($row_line["TYPE"] == 'CENTER') {
    header('Location: visual_center.php?CENTER=' . $LINE . '&BIZ=' . $_GET["BIZ"] . '&PERIOD=' . $PERIOD_GET . '');
} else {
    // somthing
}
?>
...
<body>
    ...
    FROM `startup_item` 
    WHERE LINE = '$LINE' 
    AND LastUpdate LIKE (SELECT DISTINCT CONCAT(date_format(LastUpdate, '%Y-%m-%d %H:%i'), '%') FROM `startup_item` WHERE LINE = '$LINE' ORDER BY LastUpdate DESC LIMIT 1)";
    ...
    if ($PERIOD_GET == "WEEK") {
        $STRAT_DATE = $_GET["STRAT_DATE"];
        $END_DATE = $_GET["END_DATE"];
        $query_shift_date = "AND `SHIFT_DATE` BETWEEN '$STRAT_DATE' AND '$END_DATE'";
        $date_arr = explode("-", $_GET["DATE"]);
        // echo str_replace("W","",$date_arr[1]);
        // echo date('W');
        ...
        $STRAT_DATE = "";
        $END_DATE = "";
        $query_shift_date = "AND STARTTIME LIKE '$DATE_SHIFT%'";
    }

    // echo $SHIFT_SELECT;
    if (isset($SHIFT_SELECT) && $SHIFT_SELECT != '' && $PERIOD_GET != "WEEK") {
        $strSQL = "SELECT *,
    ...
    WHERE `PERIOD` = '$PERIOD_GET' AND LINE = '$LINE' AND SHIFT_DATE LIKE '$DATE_SHIFT%' AND SHIFT LIKE '%$SHIFT_SELECT%' ORDER BY ID DESC";
    } else {
        $strSQL = "SELECT *,
    TIMESTAMPDIFF(MINUTE, STARTTIME, DATETIME1) AS TAKT1,
    TIMESTAMPDIFF(MINUTE, DATETIME1, DATETIME2) AS TAKT2,
    TIMESTAMPDIFF(MINUTE, DATETIME2, DATETIME3) AS TAKT3  FROM `$tbl_time` 
    WHERE `PERIOD` = '$PERIOD_GET' AND LINE = '$LINE' $query_shift_date ORDER BY ID DESC";
    }
    ...
    $MODEL = array();
    while ($objResult = mysqli_fetch_array($objQuery)) {
        array_push($CONFIRM1, array('MODEL' => $objResult['MODEL'], 'TYPE' => $objResult['TYPE'], 'CONFIRM1' => $objResult['CONFIRM1'], 'TAKT1' => $objResult['TAKT1'], 'DATETIME1' => $objResult['DATETIME1']));
        array_push($CONFIRM2, array('MODEL' => $objResult['MODEL'], 'TYPE' => $objResult['TYPE'], 'CONFIRM2' => $objResult['CONFIRM2'], 'TAKT2' => $objResult['TAKT2'], 'DATETIME2' => $objResult['DATETIME2']));
        array_push($CONFIRM3, array('MODEL' => $objResult['MODEL'], 'TYPE' => $objResult['TYPE'], 'CONFIRM3' => $objResult['CONFIRM3'], 'TAKT3' => $objResult['TAKT3'], 'DATETIME3' => $objResult['DATETIME3']));
    ...
    <section>
        <div class="row text-center">
            <div class="col-lg-12 mx-auto">
                ...
                <div class="mx-auto">
                    <p>
                        <span class="nowrap">LINE : <?php echo $LINE; ?></span>
                        <span class="nowrap" id="model_show"></span>
                    ...
                    <div class="row">
                        <div class="col-lg-4 mx-auto">
                            ...
                            <div class="row" id="index1">
                                ...
                                ?>
                                        <div class="col-<?php echo $container_sub ?>"><br>
                                            <h5><?php echo $TECH['TYPE']; ?></h5>
                                            <h6><?php echo $TECH['MODEL']; ?></h6>
                        ...
                        <div class="col-lg-4 mx-auto">
                                ...
                                ?>
                                        <div class="col-<?php echo $container_sub ?>"><br>
                                            ...
                                            if (empty($SUP['CONFIRM2'])) {
                                                echo '<img src="framework/img/avatar.png" width="60%">';
                                            } else {
                            ...
                            <form method="POST" action="confirm.php">
                                ...
                                <input type="hidden" name="LINE_TYPE" ID="LINE_TYPE">
                                <input type="hidden" name="MODEL" ID="MODEL_CONFIRM" value="">
                                <input type="hidden" name="CONFIRM" value="CONFIRM2">
                                <input type="hidden" name="PERIOD" value="<?php echo $PERIOD_GET ?>">
                        ...
                        <div class="col-lg-4 mx-auto">
                            ...
                            <form method="POST" action="confirm.php">
                                ...
                                <input type="hidden" name="DATE_SHIFT" value="<?php echo $DATE_SHIFT; ?>">
                                <input type="hidden" name="DATE" value="<?php echo $DATE; ?>">
                                <input type="hidden" name="LINE_TYPE3" ID="LINE_TYPE3">
                                <input type="hidden" name="MODEL" ID="MODEL_CONFIRM2" value="">
...
<?php
$insert_target = "SELECT * FROM `target_shift` WHERE LINE LIKE '$LINE'";
$objQuery = mysqli_query($con, $insert_target);
$objResult = mysqli_fetch_array($objQuery);
$TARGET1 =  $objResult['TARGET1'];
$TARGET2 = $objResult['TARGET2'];
$TARGET3 = $objResult['TARGET3'];
$TARGET_DAY = $objResult['TARGET_TIME_SHIFT_DAY'];
$TARGET_NIGHT = $objResult['TARGET_TIME_SHIFT_NIGHT'];
$i = 0;
$check = 0;
?>
...
<script>
    ...
    PERIOD = <?php echo json_encode($PERIOD_GET); ?>;
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            ...
            data: {
                ...
                'PERIOD': PERIOD // all variables i want to pass. In this case, only one.
            },
    ...
    if (BIZ == 'AC') {
        function backtohome() {
            window.location.href = 'visual.php?BIZ=IM&PERIOD=<?php echo $PERIOD_GET ?>';
        }
    } else {
        function backtohome() {
            window.location.href = 'visual.php?BIZ=IM&PERIOD=WEEK&PERIOD=<?php echo $PERIOD_GET ?>';
        }
    }
    ...
    function refresh() {
        ...
        // window.location.href = 'http://43.72.52.51/startup2.0/visual_line.php?LINE=<?php echo $LINE; ?>&DATE=<?php echo $DATE; ?>&SHIFT=<?php echo $SHIFT; ?>&DATE_SHIFT=<?php echo $DATE_SHIFT; ?>&BIZ=<?php echo $BIZ; ?>&PERIOD=<?php echo $PERIOD_GET ?>';
    }
</script>